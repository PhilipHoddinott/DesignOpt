...
[pNet,error]=handleTrainNet2(pNet,activationFunction, dActivationFunction, numberOfHiddenUnits, inputValues, targetValues, epochs, batchSize, learningRate,inputValuesTest, labelsTest);
...
function [pNet,error] = handleTrainNet2(pNet,activationFunction, dActivationFunction, numberOfHiddenUnits, inputValues, targetValues, epochs, batchSize, learningRate,inputValuesTest, labelsTest)
    %figure
    %hold on;
    %trainingSetSize=60000;
    % The number of training vectors.
    ...
    errorOld=100;
     ...
     hA = animatedline('Color','b','lineStyle','--','Marker','x');
    figure; hold on;
    grid on
    %subplot(2,1,1)
    %plot(1, 1,'*');
    ...
    for t = 1: epochs
        ...
        if mod(t,25)==0
            ...
            strT=sprintf('Epoch = %d, Accuracy = %.5f',t,acc);
            title(strT);
        end
