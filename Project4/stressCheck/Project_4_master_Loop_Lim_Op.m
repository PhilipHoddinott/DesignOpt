...
if counter>1
    r_in=sprGeo(1:end/2);
    thicc=sprGeo(end/2+1:end);
    x = (1:1:length(r_in))'; 
    v = r_in;
    xq = (1:1:nodes)';
    r_inNew = interp1(x,v,xq,'spline');
    
    x = (1:1:length(r_in))'; 
    v = thicc;
    xq = (1:1:nodes)';
    thiccNew = interp1(x,v,xq,'spline');
    for ij=1:length(r_inNew)
        if r_inNew(ij)<.01
            r_inNew(ij:end)=.01;
        end
    end
    for ij=1:length(thiccNew)
        if thiccNew(ij)<.0025
            thiccNew(ij)=.0025;
        end
    end
    ...
    figure(42069+counter)
    xFigOld=linspace(0,wing.L,nodesArr(counter-1))';
    hold on
    plot(xFigOld,sprGeo(1:end/2),'b-+')
    plot(xFigOld,sprGeo(1:end/2)+sprGeo(1+end/2:end),'b-x')
    xFig=linspace(0,wing.L,nodesArr(counter))';
    plot(xFig,r_inNew,'r-+')
    rOutNew=r_inNew+thiccNew;
    %plot(xFig,r_inNew+thiccNew,'r-x')
    plot(xFig,rOutNew,'r-x')
    legend('Old - Inner','Old - Outer','New - Inner','New - Outer')
    sTit=sprintf('Old Nx = %d, New Nx = %d',nodesArr(counter-1),nodesArr(counter));
    title(sTit)
    ylim([.01 .05])
    xlim([0 7.5])
    grid on
...
wing=Spar(w,L,rho,E,Y,nodes,x0,A,b,lb,ub,NCP,NPP);%,1e-60);
%% Start fmincon
tic % start timer

[sprMss,sprGeo,exp,var,std,wts,pts,exitFlag, fmincOutput ] =fmincon_handleStressCheck(wing);
...
expBmc(counter)={sigmaExp};
varBmc(counter)={sigmaVar};
std = sqrt(sigmaVar - sigmaExp.*sigmaExp);
counter=counter+1;
...
strSv=sprintf('lim_wkspc_Nx_%d',nodes);
%save(strSv)
toc
end
dipEnd=[];
